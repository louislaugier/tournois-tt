...
<body class="min-h-screen bg-gray-50">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <article class="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 p-6">
            <!-- Header -->
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-gray-900 mb-3">${tournament.name}</h1>
            ...
            <div class="">
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div class="space-y-4">
                        <section>
                            <h2 class="text-lg font-semibold text-gray-900 mb-3">Informations du tournoi</h2>
                            <div class="space-y-2 text-sm text-gray-600">
                                <div class="flex items-start">
                                    <svg style="width: 16px; height: 16px; margin-right: 8px; margin-top: 2px; color: #666; flex-shrink: 0;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                    </svg>
                                    <span>${formatDate(tournament.startDate)}${tournament.startDate !== tournament.endDate ? ` - ${formatDate(tournament.endDate)}` : ''}</span>
                                </div>
                                <div class="flex items-start">
                                    <svg style="width: 16px; height: 16px; margin-right: 8px; margin-top: 2px; color: #666; flex-shrink: 0;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <span>${fullAddress}${tournament.address.disambiguatingDescription ? `<br><span class="text-gray-500">${tournament.address.disambiguatingDescription}</span>` : ''}</span>
                                </div>
                            </div>
                        </section>
                    </div>
                    ...
                    <!-- Actions et liens -->
                    <div class="space-y-4">
                        ${tournament.rules?.url ? `<div class="pt-4 border-t border-gray-200">
                            <a href="${tournament.rules.url}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center text-blue-600 hover:text-blue-800 font-medium text-sm transition-colors duration-200">
                                Voir le règlement
                        ...
                        ${(tournament.page && new Date(tournament.startDate) >= new Date()) ? `<div class="pt-4 border-t border-gray-200">
                            <a href="${tournament.page}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center text-blue-600 hover:text-blue-800 font-medium text-sm transition-colors duration-200">
                                Inscription
                                <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                                </svg>
                            </a>
                        </div>` : (!tournament.page && new Date(tournament.startDate) >= new Date()) ? `<div class="pt-4 border-t border-gray-200">
                            <span class="text-gray-500 text-sm">Pas encore de lien d'inscription</span>
                        </div>` : ''}

                        <!-- Navigation -->
                        <div class="pt-4 border-t border-gray-200">
                            <div class="space-y-2">
                                <a href="/feed" class="block text-center px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 transition-colors duration-200 text-sm">
                                    ← Retour aux tournois
                                </a>
                                <a href="/" class="block text-center px-4 py-2 bg-blue-100 text-blue-700 rounded-md hover:bg-blue-200 transition-colors duration-200 text-sm">
                                    Voir sur la carte
                                </a>
                            </div>
                        </div>
